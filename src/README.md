...
- **`pipeline/`** — Multi-pass render pipeline. Entry: `MultiPassPipeline` from `./pipeline`. Contains `passes/` (Raw, Blur, BlinnPhong, Compositor) and `utils/` (fullscreen quad, scene clone helpers).
...
- **`archived/`** — Reference implementations not used in the app (e.g. `RawPassUnlit`).
